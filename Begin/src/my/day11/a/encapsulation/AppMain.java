...
public class AppMain {
	...
	static void showAllGujikja(Gujikja[] gjArr) {
		...
		static void register(Scanner sc, Gujikja[] gjArr) {
			...
			gj.setUserid(userid); //set이 public이니까 set으로 줌
			gj.setPwd(pwd);
			gj.setName(name);
		...
		public static void main(String[] args) {
			...
			gjArr[0] = new Gujikja(); // 저장소
			gjArr[0].setUserid("hongs");
			gjArr[0].setPwd("123abcD!");
			gjArr[0].setName("홍길동");
			gjArr[0].setBirthday("19970825");
			
			if(!gjArr[0].isUseGujikja()) { // 적어도 한개가 null이라면 
				gjArr[0] = null; // gjArr에 있는 거를 뺌
			}
			
			
			gjArr[1] = new Gujikja(); // 저장소
			gjArr[1].setUserid("lss");
			gjArr[1].setPwd("asd123");
			gjArr[1].setName("이순신");
			gjArr[1].setBirthday("21011111");
			
			if(!gjArr[1].isUseGujikja()) { // 적어도 한개가 null이라면 
				gjArr[1] = null; // gjArr에 있는 거를 뺌
			}
