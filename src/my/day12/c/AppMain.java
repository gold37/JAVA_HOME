...
public class AppMain {
	...
	static void loginGujikja(Scanner sc, Member[] mbrArr) {
		...
		for(int i=0; i<mbrArr.length; i++) {
			if(mbrArr[i] != null && mbrArr[i] instanceof Gujikja && mbrArr[i].isExists(id, pwd)) { // mbrArr[i] 안에는 Gujikja와 Company가 들어있음
				...
				System.out.println("구직자("+mbrArr[i].name+")로 로그인하셨습니다 (•̀ᴗ•́) و ̑\n");
	...
	static void loginCompany(Scanner sc, Member[] mbrArr) {
		...
		for(int i=0; i<mbrArr.length; i++) {
			if(mbrArr[i] != null && mbrArr[i] instanceof Company && mbrArr[i].isExists(id, pwd)) {
				System.out.println("구인회사("+mbrArr[i].name+")로 로그인하셨습니다 ୧༼◕ ᴥ ◕༽୨\n");
				mbrArr[i].showInfo();
				flag = true;
				break;
			}
	...
	public static void main(String[] args) {
		...
		Member[] mbrArr = new Member[6]; // Member mbr = new Member(); 같은 인스턴스화 불가능 ---> 미완성 클래스기때문에
		mbrArr[0] = g1;
		mbrArr[1] = c1;
		mbrArr[2] = g2;
		mbrArr[3] = c2;
